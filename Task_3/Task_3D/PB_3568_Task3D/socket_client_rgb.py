...
# Functions:	def setup_client, 	def receive_message_via_socket, def send_message_via_socket, def rgb_led_setup, def rgb_led_set_color
# 					
...
def rgb_led_setup():
	...
	GPIO.setup(redPin,GPIO.OUT)  # All pins as Outputs
...
def rgb_led_set_color(color):
	...
	##################	ADD YOUR CODE HERE	##################
	x= pwm_values[color][0]
	y= pwm_values[color][1]
	z= pwm_values[color][2]
	GPIO.output(redPin,x)
	GPIO.output(greenPin,y)
	GPIO.output(bluePin,z)
	##########################################################
